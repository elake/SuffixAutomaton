...
int main()
{
	...
	for (int i = 0; i < source.size(); i++)
	{
        ...
        for (int t = 0; t < search[i].size(); t++)
        {
            ...
            if (passed == "passed")
            {
                cout << "Searching for \"" << search[i][t].first << "\" in " << source[i].first << " PASSED. Found " << positions.size() << " of an expected " << search[i][t].second << " positions and all were validated." << endl;
            }
			else
			{
